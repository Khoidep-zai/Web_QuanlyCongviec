...
/**
 * Tạo sẵn tài khoản admin mặc định khi server khởi động.
 */
const ensureDefaultAdmin = async () => {
  try {
    ...
    if (existedAdmin) {
      ...
      existedAdmin.isActive = true;
      existedAdmin.password = adminPassword;
      await existedAdmin.save();
      console.log(`👑 Tài khoản admin đã sẵn sàng: ${adminUsername}`);
      return;
    }
    ...
    console.warn('⚠️  Không thể tạo tài khoản admin mặc định:', error.message);
  }
};
